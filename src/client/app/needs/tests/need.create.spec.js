...
/*globals $state, SharedFactory, NeedsFactory, $stateParams */
//CreatedNeedController
describe('CreatedNeedController', function() {
    ...
    beforeEach(function () {
        bard.inject(this, '$controller', '$rootScope', '$state',
            'NeedsFactory', 'SharedFactory', '$sessionStorage', '$stateParams');
        scope = $rootScope.$new();
        state = $state;
        ...
        session.token = '123';
        controller = $controller('CreatedNeedController', {
            $scope: scope
        });
    });

    //constants, initial testing
    describe('create need controller functionality', function() {
        it('should be created successfully', function () {
            expect(controller).to.be.defined;
        });
        it('should have name', function () {
            var vm = controller;
            expect(vm.title).to.equal('CreatedNeedController');
        });
        ...
        it('should have some methods from factory available', function () {
            ...
            expect(NeedsFactory.getConcreteNeed).to.be.ok;
            expect(NeedsFactory.getReponsesForThisNeed).to.be.ok;
            expect(NeedsFactory.respondToCurrentNeed).to.be.ok;
            expect(NeedsFactory.cancelUserResponse).to.be.ok;
            expect(NeedsFactory.getUserToContactWith).to.be.ok;
            expect(NeedsFactory.patchResponse).to.be.ok;
        });
        it('should allow response if user is logged in, ' +
            'is not an owner of current need and need is not closed', function () {
            var vm = controller;
            vm.userCreated.authorized = true;
